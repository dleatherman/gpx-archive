...
export const CreateSVGfromGeoJSON = ({data, strokeSize = 3, dimensions = { width: 400, height: 400 }}) => {
  if (!data) return;
  ...
  const { data: optimizedSVG } =
    optimize(`<svg class="workout-path-wrapper" xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" preserveAspectRatio="xMidYMid meet">
        <path class="workout-path" d="${paths.join("")}" fill="none" stroke="currentColor" stroke-width="${strokeSize}" />
      </svg>`);
